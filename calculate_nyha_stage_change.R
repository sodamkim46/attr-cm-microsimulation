#
# Calculate NYHA Stage Change at 30 Months for Stabilizers vs SoC
#

# clear R's session memory (Global Environment)
rm(list = ls())

### Defining model functions (Copied from TRACE_microsim_V.1.0.R) ###
...
# 4a. Update discontinuation status for patients entering NYHA4
  ...
  # Discounting and Summary (not strictly needed for this request, but kept for completeness)
...
# Define model inputs
## General parameters
num_i               <- 10000             # number of simulated individuals (as requested)
num_cycles          <- 5                 # time horizon (30 months = 5 cycles of 0.5 years)
...
# For this specific analysis, we'll use a male cohort for simplicity as requested.
...
# Starting probabilities as requested: NYHA1: 10.8%, NYHA2: 72%, NYHA3: 17.2%
p_starting <- c("NYHA1" = 0.108, "NYHA2" = 0.72, "NYHA3" = 0.172, "NYHA4" = 0, "Death" = 0)
...
# 1. Stabilizers (Tafamidis) - Time-Varying (Lines 494-522 from TRACE_microsim_V.1.0.R)
...
# 2. Standard of Care (SoC) - Time-Varying (Lines 563-591 from TRACE_microsim_V.1.0.R)
...
cat("\n--- Running Simulation for Stabilizers Arm (Tafamidis) ---\n")
...
cat("\n--- Running Simulation for Standard of Care (SoC) Arm (Placebo) ---\n")

# To simulate a pure SoC arm, we set treatment-related parameters to SoC values
# and set discontinuation probability to 0.
res_soc <- run_microSimV_hosp_disc(
  v_starting_states   = v_starting_states,
  num_i               = num_i,
  num_cycles          = num_cycles,
  m_indi_features     = m_indi_features_male,
  v_states_names      = v_states_names,
  v_states_costs_arm  = v_states_costs_soc, # Use SoC costs
  v_states_costs_soc  = v_states_costs_soc, # Use SoC costs
  v_states_utilities  = v_states_utilities,
  l_trans_probs_arm   = l_trans_probs_soc_tv, # Use SoC transition probabilities
  l_trans_probs_soc   = l_trans_probs_soc_tv, # Use SoC transition probabilities
  p_disc              = 0, # No discontinuation from SoC
  params_gompertz_sex = params_gompertz$male,
  v_hosp_probs        = v_hosp_probs_st, # Using stabilizer hosp probs as baseline (assumption)
  v_hosp_costs        = v_hosp_costs,
  v_hosp_disutils     = v_hosp_disutils,
  discount_rate_costs = discount_rate_costs,
  discount_rate_QALYs = discount_rate_QALYs,
...
# Define NYHA stage order for comparison
nyha_order_map <- c("NYHA1" = 1, "NYHA2" = 2, "NYHA3" = 3, "NYHA4" = 4, "Death" = 5)

# Function to calculate the percentage of individuals who stayed the same or improved
calculate_improvement_percentage <- function(m_States, nyha_order_map) {
  initial_states <- m_States[, 1]
  final_states <- m_States[, ncol(m_States)] # State at 30 months (cycle 5, which is column 6)
  
  initial_numeric <- nyha_order_map[initial_states]
  final_numeric <- nyha_order_map[final_states]
  
  # Count individuals where final numeric stage is less than or equal to initial numeric stage
  # (e.g., NYHA1 (1) <= NYHA2 (2) is improvement, NYHA2 (2) <= NYHA2 (2) is same)
  improved_or_same_count <- sum(final_numeric <= initial_numeric)
  
  percentage <- (improved_or_same_count / length(initial_states)) * 100
  return(percentage)
}

# Calculate for Stabilizers (Tafamidis) arm
percentage_st <- calculate_improvement_percentage(res_st$m_States, nyha_order_map)

# Calculate for SoC (Placebo) arm
percentage_soc <- calculate_improvement_percentage(res_soc$m_States, nyha_order_map)

cat("\n--- NYHA Stage Change Results at 30 Months ---\n")
cat(sprintf("Tafamidis Arm: %.2f%% of individuals stayed the same or improved their NYHA stage.\n", percentage_st))
cat(sprintf("Placebo (SoC) Arm: %.2f%% of individuals stayed the same or improved their NYHA stage.\n", percentage_soc))
